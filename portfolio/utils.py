...
def fetch_prices_and_currencies(assets):
    ...
    zero_assets = [asset for asset in assets if (asset.is_tradable == False or (asset.type_asset == 'CY' and asset.isin is None and asset.figi is None))]
    if zero_assets:
        for asset in zero_assets:
            prices_and_currencies[asset.id] = {'price': 0, 'currency': asset.currency_base_settlement.ticker}
